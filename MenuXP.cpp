...
int CMenuXP::PerentMenuSize = 0;
//узнаем размер родителя меню
void CMenuXP::GetMenuParentWndSize(CWnd* pWndPerent)
{
	CRect MenuPerentRect;
	pWndPerent->GetWindowRect(&MenuPerentRect);

	PerentMenuSize = MenuPerentRect.Width() - 18;
}
///////////////////////////////////////////////////////////////////////////////
CMenuItem::CMenuItem ()
{
...
bool CMenuItem::Draw (CDC* pDC, LPCRECT pRect, bool bSelected, bool bMenuBar, bool bHotLight, bool bInactive, bool bNoAccel) const
{
	...
	static BOOL bGradientSel = AfxGetApp()->GetProfileInt("MenuEx", "ShowGradientSel", FALSE);
	//static COLORREF crGradientStart=AfxGetApp()->GetProfileInt("MenuXP", "GradientStart", RGB(0xB4, 0xC4, 0xE4)); //origin color
	static COLORREF crGradientStart = AfxGetApp()->GetProfileInt("MenuXP", "GradientStart", RGB(200, 200, 200));
	//static COLORREF crGradientEnd=AfxGetApp()->GetProfileInt("MenuXP", "GradientEnd", RGB(0xFF, 0xFF, 0xFF)); //origin color
	static COLORREF crGradientEnd = AfxGetApp()->GetProfileInt("MenuXP", "GradientEnd", RGB(245, 245, 245)); //мой color
...
//	static COLORREF crGradientEnd=AfxGetApp()->GetProfileInt("MenuXP", "GradientEnd", -1);
	...
	//static int nGradientWidth = AfxGetApp()->GetProfileInt("MenuXP", "GradientWidth", IMGWIDTH + IMGWIDTH - 2); //ширина левого градиента
	static int nGradientWidth = 0;
    ...
    {
		//тут рисуем выделение пункта меню для панели меню
		TRACE("рисуем выделение пункта меню для панели меню -> CMenuItem::Draw()\n");
    ...
    {
		//тут рисуем выделение пункта меню во всплывающем меню
		TRACE("рисуем выделение пункта меню во всплывающем меню -> CMenuItem::Draw()\n");
        ...
        else
        {
			...
			{
				//рисуем градиент заливки пункта меню
				DrawGradient(pDC,
    ...
    else if ( !bMenuBar )
    {
		...
		//если меню будет с градиентом
		if (bGradient && pDC->GetDeviceCaps(BITSPIXEL) > 8)
		{
			//устанавливаем ширину градиента
	        rc.right = rc.left + nGradientWidth; // nGradientWidth = 0
			...
			//начинаем заполнять прямоугольник там где закончили рисовать градиент
			rc.left = rc.right;
    ...
    else
    {
		...
		//если есть название пункта меню
        if ( GetCaption (sCaption) > 0 )
        {
            ...
            else
            {
				...
				{
					//устанавливаем ширину градиента
					rc.left += nGradientWidth; // nGradientWidth = 0
					//rc.left += IMGWIDTH + IMGPADDING + TEXTPADDING;
				}

				//pDC->DrawText (sCaption, rc, DT_SINGLELINE|DT_VCENTER|DT_LEFT|dwHidePrefix); //рисуем текст с выравниванием по левому краю
				pDC->DrawText (sCaption, rc, DT_SINGLELINE|DT_VCENTER|DT_CENTER|dwHidePrefix); //выравниваем текст по середине
...
void CMenuXP::OnMeasureItem (MEASUREITEMSTRUCT* pMeasureItemStruct)
{
    ...
    if ( pMeasureItemStruct->CtlType == ODT_MENU )
    {
        ...
        { 
			//вычисляем ширину пункта меню
			//int itemWidth = 4; //by default itemWidth = 4
            //pMeasureItemStruct->itemWidth = IMGWIDTH + IMGPADDING + HIWORD(pMeasureItemStruct->itemData) + TEXTPADDING + HIBYTE(LOWORD(pMeasureItemStruct->itemData)) + TEXTPADDING + itemWidth;
			
			//мне нужно установить ширину пункта меню равной ширине кнопки
			pMeasureItemStruct->itemWidth = PerentMenuSize;
        }
    }
}
...
void CWndMenuXP::OnPrint (CDC* pDC, bool bOwnerDrawnItems)
{
    ...
    if ( GetWinVersion() < wvWinXP )
    {
        ...
        CDC cdcMemDC;
        cdcMemDC.CreateCompatibleDC (pDC);
        HGDIOBJ hOldBitmap = ::SelectObject (cdcMemDC.m_hDC, m_bmpBkGnd);
        pDC->BitBlt (0, rc.bottom-SM_CXSHADOW, SM_CXSHADOW*2, SM_CXSHADOW, &cdcMemDC, 0, rc.bottom-SM_CXSHADOW, SRCCOPY);
        pDC->BitBlt (rc.right-SM_CXSHADOW, rc.bottom-SM_CXSHADOW, SM_CXSHADOW, SM_CXSHADOW, &cdcMemDC, rc.right-SM_CXSHADOW, rc.bottom-SM_CXSHADOW, SRCCOPY);
        pDC->BitBlt (rc.right-SM_CXSHADOW, 0, SM_CXSHADOW, SM_CXSHADOW*2, &cdcMemDC, rc.right-SM_CXSHADOW, 0, SRCCOPY);
        DrawShadow (cdcMemDC.m_hDC, pDC->m_hDC, rc);
        ::SelectObject (cdcMemDC.m_hDC, hOldBitmap);
    }
}
