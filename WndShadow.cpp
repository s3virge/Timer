...
void CWndShadow::Show(HWND hParentWnd)
{
	...
	//так как мое окно по€вл€етс€ постепенно, то нунжо показать тень сразу, но полностью прозрачной
	//прозрачность тени будет измен€тьс€ у мен€ в CTimerDlg::GraduallyShowHideWindow()
	//вызовом m_Shadow.SetDarkness(c);
	m_Status |= SS_VISABLE;
